...
ls -lh "$BUILD_DIR"/tunnel-rs-linux-* 2>/dev/null || echo "  (none found)"
echo ""
...
if command -v file &> /dev/null; then
    file "$BUILD_DIR"/tunnel-rs-linux-* "$BUILD_DIR"/tunnel-rs-ice-* "$BUILD_DIR"/tunnel-rs-vpn-* 2>/dev/null || true
else
