...
/// Default QUIC receive window size (64 MB).
pub const DEFAULT_RECEIVE_WINDOW: u32 = 64 * 1024 * 1024;
...
pub struct TransportTuning {
    ...
    /// QUIC receive window size in bytes (default: 67108864 = 64MB).
    /// Controls flow control - larger values allow more in-flight data.
    /// Valid range: 1024 to 268435456 (256MB).
...
/// Maximum QUIC window size (256 MB).
const MAX_WINDOW_SIZE: u32 = 256 * 1024 * 1024;

/// Validate QUIC window size is within acceptable range (1024-268435456 bytes).
fn validate_window_size(size: u32, field_name: &str, section: &str) -> Result<()> {
    ...
    if size > MAX_WINDOW_SIZE {
        anyhow::bail!(
            "[{}] {} value {} exceeds maximum of {} bytes (256MB)",
            section,
            field_name,
            size,
            MAX_WINDOW_SIZE
