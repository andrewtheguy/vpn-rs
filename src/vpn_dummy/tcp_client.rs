...
//! performs the same length-prefixed handshake the iroh client uses, then runs
//! the shared [`run_tunnel`] pipeline over the TCP socket.
...
pub async fn run_dummy_client(server: SocketAddr, mtu_override: Option<u16>) -> VpnResult<()> {
    ...
    let handshake = VpnHandshake::new(device_id);
    ...
    // offload (mirrors the iroh client).
