...
//!
//! Accepts one client at a time over plain TCP, performs the length-prefixed
//! handshake (auth skipped), assigns a static IPv4, then runs the shared
//! [`run_tunnel`] pipeline. Because [`run_tunnel`] takes no `self` and only
//! shovels packets between the TUN device and the transport, the dummy server is
//! the exact mirror of the dummy client — the same hot path on both ends.
...
use crate::vpn_core::signaling::{
    ...
    VpnHandshakeResponse, MAX_CAPABILITIES_PAYLOAD, MAX_HANDSHAKE_SIZE,
};
...
async fn handle_connection(
    ...
    // Create the server's own TUN (its address is the gateway).
    ...
    let response =
        VpnHandshakeResponse::accepted(client_ip, network, server_ip, server_gso, mtu);
    ...
    run_tunnel(tun, write_half, read_half, client_caps.gso_enabled).await
}
