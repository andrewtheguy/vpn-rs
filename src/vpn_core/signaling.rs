...
pub fn frame_ip_packet_v2(
    ...
    buf.clear();
    append_ip_packet_v2(buf, offload, ip_packet).map(|_| ())
}

/// Append a framed IP packet (v2 layout, same as [`frame_ip_packet_v2`]) to `buf`
/// without clearing it, returning the number of bytes written.
///
