//! Buffered frame reader for the stream (TCP) VPN data channel.
//!
//! Reads protocol headers exactly and fills payload buffers directly from the
//! byte stream. Payloads are read into `BytesMut` spare capacity via
//! `ReadBuf::uninit`, avoiding zero-fill before the stream overwrites those
//! bytes.
//!
//! The UDP transport delimits messages by datagram boundary (see
//! [`crate::vpn_core::datagram::classify`]); this reader instead splits
//! length-prefixed frames out of a continuous TCP byte stream.
...
pub enum FrameError {
    ...
    FrameTooLarge(usize),
    /// Underlying stream read error or truncated frame at end of stream.
    Read(String),
}
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Read(e) => write!(f, "stream read error: {}", e),
        }
    }
}
...
///
/// Works with `ReadBuf::uninit` so payload storage can be filled without a
/// zeroing pass.
...
///
/// Generic over any [`AsyncRead`] source — for the dummy TCP benchmark
/// transport this is a plain `tokio::net::tcp::OwnedReadHalf`. The only
/// requirement is `AsyncRead + Unpin`.
...
impl<R: AsyncRead + Unpin> FrameReader<R> {
    ...
    async fn read_exact_stack(&mut self, buf: &mut [u8], context: &str) -> Result<(), FrameError> {
    ...
    async fn read_frame_payload(&mut self, frame_len: usize) -> Result<Bytes, FrameError> {
        ...
        // Reading straight into `spare_capacity_mut` lets us build an owned,
        // zero-copy `Bytes` from the reused arena without the memset that a safe
        // `resize(frame_len, 0)` + `read_exact` would incur on every frame (the
        // no-zero-fill goal stated in this module's doc comment).
        //
...
mod tests {
    ...
    async fn frame_reader_reads_over_generic_async_read() {
        // Proves FrameReader works over a generic AsyncRead (here a duplex
