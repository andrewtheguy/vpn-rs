//! Transport-agnostic chunked writer for the stream (TCP) VPN data channel.
//!
//! The hot-path writer task drains a batch of framed packets (`&mut [Bytes]`)
//! and writes them to the transport in one go. This trait lets the shared
//! [`run_tunnel`](crate::vpn_core::tunnel::run_tunnel) packet loop stay generic
//! over the write half. The TCP transport, used by the dummy benchmark mode,
//! sends the batch with vectored `writev` so it issues roughly one syscall per
...
/// tokio task. The `chunks` slice may be mutated by the implementation, so
/// callers must `clear()` and refill it rather than relying on its contents
/// after the call.
...
impl ChunkedWrite for tokio::net::tcp::OwnedWriteHalf {
    async fn write_all_chunks(&mut self, chunks: &mut [Bytes]) -> io::Result<()> {
        ...
        // the socket accepts it all) rather than one `write_all` syscall per
        // frame. A cursor (chunk index + byte offset into that chunk) tracks
        // progress so partial writes just resume from where `writev` stopped.
        let mut chunk_idx = 0;
        let mut offset = 0;
