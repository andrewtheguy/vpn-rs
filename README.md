...
>
> **Alternative for signaling-only:** Use [nostr mode](docs/ALTERNATIVE-MODES.md#nostr-mode) with self-hosted Nostr relays. Nostr relays only handle signaling (small encrypted messages), never tunnel traffic. If hole punching fails, the connection fails — no traffic is ever forwarded through the relay.
